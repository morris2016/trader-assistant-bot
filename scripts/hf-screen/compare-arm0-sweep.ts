// "Arm-at-0" trail variant: trail arms the moment the trade goes positive,
// instead of waiting for +1×ATR favorable. Tests different retrace widths
// + optional fixed TP cap. Last N hours (default 48).
//
// Configs tested (all keep TP=2×ATR fixed cap and SL=1×ATR):
//   A. Arm=0,  retrace 0.3×ATR   (locks tiny wins, aggressive)
//   B. Arm=0,  retrace 0.5×ATR
//   C. Arm=0,  retrace 1.0×ATR   (≈ "exit on any back-to-entry move")
//   D. Arm=0,  retrace 0.3×ATR  + NO TP (let the trail ride)
//   E. Arm=1.0,retrace 0.3×ATR  + TP=2×ATR (= current hybrid, control)
//   F. Pure Fixed 2:1 (control)
//
// Run: HOURS=48 npx tsx scripts/hf-screen/compare-arm0-sweep.ts

import * as fs from "fs";
import {
  ASSETS, COST_RT, HARD_TIMEOUT_MIN, HARD_SL_ATR,
...
// Generic exit sim. armMult=0 means arm-from-entry. tpMult=null means no TP cap.
function simExit(
  bars1m: Bar[], startIdx: number, entry: number, atr: number, side: Side,
  armMult: number, retraceMult: number, tpMult: number | null, slMult: number,
): { exitPx: number; closeEpoch: number; reason: "TP" | "SL" | "trail" | "timeout" | "open" } {
  const armD = armMult * atr;
  const trailD = retraceMult * atr;
  const slD = slMult * atr;
  const tpD = tpMult !== null ? tpMult * atr : Infinity;
  const slPx = side === "LONG" ? entry - slD : entry + slD;
  const tpPx = tpMult !== null ? (side === "LONG" ? entry + tpD : entry - tpD) : NaN;
  let peak = entry, armed = armMult === 0 ? false : false;  // armed becomes true once peak >= entry + armD
  ...
  for (let i = startIdx + 1; i <= maxIdx; i++) {
    ...
    if (side === "LONG") {
      ...
      if (tpMult !== null && b.high >= tpPx) return { exitPx: tpPx, closeEpoch: b.epoch, reason: "TP" };
      if (b.high > peak) peak = b.high;
      if (!armed && peak >= entry + armD) armed = true;
      if (armed && b.low <= peak - trailD) {
        // arm-at-0 with retrace: exit price = peak - trailD. If armMult=0 and retrace=0.3, this could be below entry.
        return { exitPx: peak - trailD, closeEpoch: b.epoch, reason: "trail" };
      }
    } else {
      if (b.high >= slPx) return { exitPx: slPx, closeEpoch: b.epoch, reason: "SL" };
      if (tpMult !== null && b.low <= tpPx) return { exitPx: tpPx, closeEpoch: b.epoch, reason: "TP" };
      if (b.low < peak) peak = b.low;
      if (!armed && peak <= entry - armD) armed = true;
      if (armed && b.high >= peak + trailD) {
        return { exitPx: peak + trailD, closeEpoch: b.epoch, reason: "trail" };
      }
    }
  }
  const reason = maxIdx < trueTimeout ? "open" : "timeout";
...
async function main() {
  ...
  console.log(`\n══ Arm-at-0 trail sweep (${HOURS_BACK}h) — ${startStr} → ${endStr} UTC ══`);
  ...
  type Cfg = { id: string; arm: number; retr: number; tp: number | null; sl: number };
  const cfgs: Cfg[] = [
    { id: "A. Arm=0 / retr=0.3 / TP=2 / SL=1", arm: 0,   retr: 0.3, tp: 2.0, sl: 1.0 },
    { id: "B. Arm=0 / retr=0.5 / TP=2 / SL=1", arm: 0,   retr: 0.5, tp: 2.0, sl: 1.0 },
    { id: "C. Arm=0 / retr=1.0 / TP=2 / SL=1", arm: 0,   retr: 1.0, tp: 2.0, sl: 1.0 },
    { id: "D. Arm=0 / retr=0.3 / no TP / SL=1", arm: 0,   retr: 0.3, tp: null, sl: 1.0 },
    { id: "E. Arm=1 / retr=0.3 / TP=2 / SL=1 (current hybrid)", arm: 1.0, retr: 0.3, tp: 2.0, sl: 1.0 },
    { id: "F. Pure Fixed 2:1 (TP=2 / SL=1, no trail)", arm: 999, retr: 999, tp: 2.0, sl: 1.0 },
  ];

  type Variant = { filter: boolean; cfg: Cfg };

  function runVariant(v: Variant) {
    let wallet = START_WALLET, locked = 0;
    type Op = { asset: string; ruleId: string; side: Side; entry: number; lev: number; openEpoch: number; closeEpoch: number; stake: number; exitPx: number; reason: string };
    const open: Op[] = [];
    type Trade = Op & { pnl: number };
    const trades: Trade[] = [];
    const sigs: Signal[] = [];
    ...
    for (const sig of sigs) {
      ...
      const exit = simExit(data.bars1m, startIdx, sig.entryPx, sig.atr, sig.side, v.cfg.arm, v.cfg.retr, v.cfg.tp, v.cfg.sl);
      open.push({ asset: sig.asset, ruleId: sig.ruleId, side: sig.side, entry: sig.entryPx, lev, openEpoch: sig.nextBarEpoch, closeEpoch: exit.closeEpoch, stake, exitPx: exit.exitPx, reason: exit.reason });
  ...
  console.log(`${"Config".padEnd(50)} ${"Filt".padStart(4)} ${"N".padStart(3)} ${"WR%".padStart(5)} ${"TP".padStart(3)} ${"SL".padStart(3)} ${"trail".padStart(5)} ${"open".padStart(4)} ${"Net $".padStart(7)} ${"Realiz".padStart(8)}`);
  for (const cfg of cfgs) {
    for (const filter of [false, true]) {
      const r = runVariant({ filter, cfg });
      ...
      console.log(`${cfg.id.padEnd(50)} ${(filter ? "ON" : "off").padStart(4)} ${String(r.trades.length).padStart(3)} ${wr.toFixed(0).padStart(5)} ${String(reasons.TP).padStart(3)} ${String(reasons.SL).padStart(3)} ${String(reasons.trail).padStart(5)} ${String(reasons.open).padStart(4)} ${(r.wallet - START_WALLET).toFixed(2).padStart(7)} ${realized.toFixed(2).padStart(8)}`);
    }
  }
}
