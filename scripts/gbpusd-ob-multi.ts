// GBP/USD OB multi-TF tiered search.
// GBP is more volatile than EUR/USD; news-driven spikes (BOE, UK politics).
// Tier 1: 15m, 30m, 1h with 14 core variants + 4 GBP-specific (wider stops, news-skip).
...
const tier1Variants: Variant[] = [
  ...
  // GBP-specific: wider stop (news spike survival), tighter retest (sharp reversals)
  { name: "GBP: wideBuf·3:1",             params: obParams({ stopBufferAtrMul: 0.2 }),           filters: {} },
  { name: "GBP: wideBuf+ce·4:1",          params: obParams({ stopBufferAtrMul: 0.2, entryDepth: 1, targetRMult: 4 }), filters: {} },
  { name: "GBP: retestBars=1·3:1",        params: obParams({ retestConfirmationBars: 1 }),       filters: {} },
  { name: "GBP: maxAdx=40·4:1",           params: obParams({ targetRMult: 4 }),                  filters: { maxAdx: 40 } },
];
