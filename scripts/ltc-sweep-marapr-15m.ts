...
// OUT-OF-SAMPLE: Mar 1 → Apr 27, 2026 (~57 days, no overlap with Jan-Feb 15m test)
// All warmup bars come from Jan 13 → Feb 28 (~47d). Trades count only Mar 1+.
const WINDOW_START_EPOCH = Math.floor(new Date('2026-03-01T00:00:00Z').getTime() / 1000);
const WINDOW_END_EPOCH   = Math.floor(new Date('2026-04-28T00:00:00Z').getTime() / 1000);

async function main() {
  const c = new C(); await c.ready;
  console.log(`[ltc-sweep-marapr-15m] Symbol: Litecoin / USD (cryLTCUSD) · cost ${COST_BPS} bps`);
  console.log(`Out-of-sample window: 2026-03-01 → 2026-04-27 (~57d) with prior history as warm-up\n`);
