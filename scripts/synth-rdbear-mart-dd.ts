// RDBEAR breakout 5m — martingale + session-DD circuit.
// Mirrors production Fast2 session-DD safety: when balance falls more than
// DD_FRAC below the running peak, pause new opens. Open positions still
// settle. Operator-style auto-resume disabled — once tripped, it stays tripped
// for the remainder of the window (matches the bot's manual-resume behavior).
...
const END_HOUR = Number(process.env.END_HOUR ?? 30);
...
type Trade = { sigT: string; closeT: string; level: number; stake: number; exit: "tp" | "sl"; net: number; bal: number; ddPct: number };

function honestMartSimDD(candles: Candle[], v: Variant): { trades: Trade[]; bust: boolean; ddTripped: boolean; ddTrippedAt: string | null; finalBal: number; peak: number; trough: number } {
  ...
  let bust = false;
  let ddTripped = false;
  let ddTrippedAt: string | null = null;
  for (const sig of sigs) {
    ...
    trades.push({
      ...
      closeT,
      level, stake, exit, net, bal: balance, ddPct,
    });
    if (ddPct >= DD_FRAC && !ddTripped) {
      ddTripped = true;
      ddTrippedAt = closeT;
    }
    level = exit === "tp" ? 0 : level + 1;
  }
  return { trades, bust, ddTripped, ddTrippedAt, finalBal: balance, peak, trough };
}

async function main() {
  const dateStr = new Date(TARGET_DAY_START * 1000).toISOString().slice(0, 10);
  console.log(`HONEST mart sim + session-DD circuit — RDBEAR breakout 5m`);
  console.log(`${dateStr} ${START_HOUR}:00→${END_HOUR}:00 UTC  (${(END_HOUR-START_HOUR)}h window)`);
  console.log(`ACCT=$${ACCT}  BASE=$${BASE_STAKE}  MART=${MART}× × ${MAX_LEVELS}L  CAP=$${PER_TRADE_CAP}  DD=${(DD_FRAC*100).toFixed(0)}%\n`);
  ...
  for (const v of VARIANTS) {
    const r = honestMartSimDD(candles, v);
    console.log(`══════════════════════════════════════════════════════════════════════════════════════`);
    console.log(`${v.name}`);
    console.log(`══════════════════════════════════════════════════════════════════════════════════════`);
    ...
    const status = r.bust ? "💀 BUST" : r.ddTripped ? "⏸ DD TRIPPED" : "✓";
    ...
    if (r.ddTripped) console.log(`  DD circuit fired at ${r.ddTrippedAt} — peak $${r.peak.toFixed(2)} → balance went below ${(1-DD_FRAC)*100}% of peak`);
    const maxLevel = Math.max(...r.trades.map((t) => t.level));
    console.log(`  max ladder level: ${maxLevel}`);
    console.log(`\n  ${"sigT".padEnd(8)}  ${"closeT".padEnd(8)}  lvl  ${"stake".padStart(6)}  exit   ${"net".padStart(8)}  ${"bal".padStart(8)}  ${"DD%".padStart(5)}`);
    for (const t of r.trades) {
      const ddTag = t.ddPct >= DD_FRAC ? " ⏸" : "";
      console.log(`  ${t.sigT}  ${t.closeT}   ${t.level}   $${t.stake.toFixed(2).padStart(5)}  ${t.exit}    ${(t.net >= 0 ? "+" : "") + "$" + t.net.toFixed(2).padStart(6)}    $${t.bal.toFixed(2).padStart(7)}   ${(t.ddPct*100).toFixed(0).padStart(3)}%${ddTag}`);
    }
    console.log("");
  }
}
