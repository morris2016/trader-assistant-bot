// Ethereum OB deep-dive — same systematic process as Silver / BTC.
// ETH baseline: 87 trades / 30% WR / -0.16R / +$3 on Silver-loose config (flat-to-noise).
// Earlier cross-symbol scan suggested ETH was less broken than BTC → worth testing.
//
// Expectation: like BTC, R-expectancy may underweight $ outcomes due to Deriv's
// −100% loss cap on volatile crypto bars. Trust $ on ETH too.
...
const variants: Variant[] = [
  ...
  // Stricter displacement (the BTC OB hint that didn't pan out — try anyway for ETH)
  ...
  // ADX gates (ETH might want different regime than Silver/BTC)
  { name: "loose · 3:1 + maxAdx=22 (Silver-style)",     params: basePs(),                                                  filters: { maxAdx: 22 } },
  { name: "loose · 3:1 + minAdx=22 (trending only)",    params: basePs(),                                                  filters: { minAdx: 22 } },
  { name: "loose · 3:1 + with-trend@20",                params: basePs(),                                                  filters: { withTrendOnlyAboveAdx: 20 } },
  { name: "loose · 3:1 + with-trend@22",                params: basePs(),                                                  filters: { withTrendOnlyAboveAdx: 22 } },
  // FVG-required
  ...
  // Quality filter
  { name: "loose · 3:1 + qualityLB=10",                 params: basePs({ qualityFilterLookback: 10 }),                     filters: {} },
  // CE entry
  ...
  { name: "disp1.5 + with-trend@20 · 3:1",              params: basePs({ displacementAtrMultiplier: 1.5 }),                filters: { withTrendOnlyAboveAdx: 20 } },
  { name: "loose · 3:1 + maxAdx=22 + qualityLB=10 (Silver winner)", params: basePs({ qualityFilterLookback: 10 }),         filters: { maxAdx: 22 } },
];
...
async function main() {
  ...
  console.log(`[eth-ob-deepdive] Symbol: Ethereum / USD · cost ${COST_BPS} bps`);
