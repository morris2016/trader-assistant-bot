// BOOM 300N/600/900 SELL-only — fade up-spikes (the validated direction).
// Mirror script of synth-crash-sellonly.ts but for BOOM family.
...
// SELL-only fade of BOOM up-spike (validated direction).
function detect(candles: Candle[]): Sig[] {
  ...
  for (let i = ATR_PERIOD + 2; i < candles.length; i++) {
    ...
    if (!(confirm.close < spike.close)) continue;    // confirmation bar reverses down
...
async function main() {
  console.log(`BOOM SELL-only spike-fade (validated direction) @ 1m`);
