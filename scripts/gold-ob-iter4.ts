...
const variants: Variant[] = [
  // ITER3 WIN baseline (qualifying): edge · 4:1 + BUY = +$351 / 31t / 35% WR / +0.70R
  { name: "WIN: edge · 4:1 + BUY",                       params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true } },
  // Fine R:R around 4:1
  { name: "edge · 3.5:1 + BUY",                          params: basePs({ targetRMult: 3.5 }),                              filters: { buyOnly: true } },
  { name: "edge · 4.5:1 + BUY",                          params: basePs({ targetRMult: 4.5 }),                              filters: { buyOnly: true } },
  { name: "edge · 4.0:1 + BUY + skipSat",                params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true, skipDaysOfWeekUtc: [6] } },
  { name: "edge · 4.0:1 + BUY + skipSun",                params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true, skipDaysOfWeekUtc: [0] } },
  { name: "edge · 4.0:1 + BUY + minAdx=22",              params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true, minAdx: 22 } },
  { name: "edge · 4.0:1 + BUY + maxAdx=40",              params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true, maxAdx: 40 } },
  { name: "edge · 4.0:1 + BUY + maxAdx=50",              params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true, maxAdx: 50 } },
  { name: "edge · 4.0:1 + BUY + with-trend@20",          params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true, withTrendOnlyAboveAdx: 20 } },
  // Displacement quality on top of edge·4:1+BUY
  { name: "edge · 4:1 + BUY + disp0.6",                  params: basePs({ targetRMult: 4.0, displacementAtrMultiplier: 0.6 }), filters: { buyOnly: true } },
  { name: "edge · 4:1 + BUY + disp1.0",                  params: basePs({ targetRMult: 4.0, displacementAtrMultiplier: 1.0 }), filters: { buyOnly: true } },
  { name: "edge · 4:1 + BUY + disp1.2",                  params: basePs({ targetRMult: 4.0, displacementAtrMultiplier: 1.2 }), filters: { buyOnly: true } },
  { name: "edge · 4:1 + BUY + disp1.5",                  params: basePs({ targetRMult: 4.0, displacementAtrMultiplier: 1.5 }), filters: { buyOnly: true } },
  // Lookback / search
  { name: "edge · 4:1 + BUY + lookback8",                params: basePs({ targetRMult: 4.0, lookback: 8 }),                 filters: { buyOnly: true } },
  { name: "edge · 4:1 + BUY + lookback16",               params: basePs({ targetRMult: 4.0, lookback: 16 }),                filters: { buyOnly: true } },
  { name: "edge · 4:1 + BUY + obSearchMaxBack=5",        params: basePs({ targetRMult: 4.0, obSearchMaxBack: 5 }),          filters: { buyOnly: true } },
  // Retest confirmation bars
  { name: "edge · 4:1 + BUY + retestBars=1",             params: basePs({ targetRMult: 4.0, retestConfirmationBars: 1 }),   filters: { buyOnly: true } },
  { name: "edge · 4:1 + BUY + retestBars=3",             params: basePs({ targetRMult: 4.0, retestConfirmationBars: 3 }),   filters: { buyOnly: true } },
  // Rejection body strictness
  { name: "edge · 4:1 + BUY + rejBody=0.5",              params: basePs({ targetRMult: 4.0, rejectionBodyAtrMul: 0.5 }),    filters: { buyOnly: true } },
  { name: "edge · 4:1 + BUY + rejBody=0.7",              params: basePs({ targetRMult: 4.0, rejectionBodyAtrMul: 0.7 }),    filters: { buyOnly: true } },
  // ATR period
  { name: "edge · 4:1 + BUY + atrP=10",                  params: basePs({ targetRMult: 4.0, atrPeriod: 10 }),               filters: { buyOnly: true } },
  { name: "edge · 4:1 + BUY + atrP=20",                  params: basePs({ targetRMult: 4.0, atrPeriod: 20 }),               filters: { buyOnly: true } },
  // Combined refinements (best-guess stacks)
  { name: "edge · 4:1 + BUY + disp1.2 + skipSat",        params: basePs({ targetRMult: 4.0, displacementAtrMultiplier: 1.2 }), filters: { buyOnly: true, skipDaysOfWeekUtc: [6] } },
  { name: "edge · 4:1 + BUY + disp1.0 + maxAdx=40",      params: basePs({ targetRMult: 4.0, displacementAtrMultiplier: 1.0 }), filters: { buyOnly: true, maxAdx: 40 } },
  { name: "edge · 4:1 + BUY + minAdx=18",                params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true, minAdx: 18 } },
  { name: "edge · 4:1 + BUY + minAdx=20",                params: basePs({ targetRMult: 4.0 }),                              filters: { buyOnly: true, minAdx: 20 } },
];
