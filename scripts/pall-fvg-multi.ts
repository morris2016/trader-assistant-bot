// Palladium/USD FVG multi-TF tiered search.
// Metals have been FVG winners (Silver minGap=0.7, Gold minAdx=24, Plat minAdx=24).
// Palladium is the most volatile metal, lowest liquidity — expect noisier output.
// Tier 1: 15m, 30m, 1h. Variants: minGap sweep, R:R sweep, ADX gates, BUY/SELL,
// plus PD-specific (lower minGap for thin-liquidity gaps, requireRejection for whipsaws).
...
const tier1Variants: Variant[] = [
  ...
  // Phase F: PD-specific (thin-liquidity = wider stop, requireRejection to filter whipsaws)
  { name: "PD: wideBuf·3:1",               params: fvgParams({ stopBufferAtrMul: 0.25 }),         filters: {} },
  { name: "PD: rej·3:1",                   params: fvgParams({ requireRejection: 1 }),            filters: {} },
  { name: "PD: rej+minAdx=24·4:1",         params: fvgParams({ requireRejection: 1, targetRMult: 4 }), filters: { minAdx: 24 } },
  { name: "PD: minGap=0.30+rej·4:1",       params: fvgParams({ minGapAtrMul: 0.30, requireRejection: 1, targetRMult: 4 }), filters: {} },
];
...
const TFS = [
  { gr: 3600,  label: "1h",   fetchCount: 4000 },
];

async function main() {
  const c = new C(); await c.ready;
  console.log(`Palladium/USD FVG tier-1 search · ${tier1Variants.length} variants × ${TFS.length} TFs\n`);
  ...
  console.log(`╔══════════════════════════════════════════════════════════════════════════════╗`);
  console.log(`║  Palladium/USD FVG TIER-1 VERDICT                                             ║`);
  console.log(`╚══════════════════════════════════════════════════════════════════════════════╝`);
  ...
  if (totalPassers === 0) console.log(`\n❌ Tier-1 found nothing. Recommend dropping Palladium/USD FVG.`);
  else {
