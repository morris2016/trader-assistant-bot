// Iteration 2: refine around the FVG winner (1h, edge entry, 3:1, minGap=0.5).
...
const GRANULARITY = 3600; const COUNT = 3000;
...
function basePs(over: Record<string, number> = {}): Record<string, number> {
  return {
    atrPeriod: 14, minGapAtrMul: 0.5, maxActive: 12,
...
const variants: Variant[] = [
  { name: "WIN: edge · 3:1 · minGap=0.5",                    params: basePs(),                                                        filters: {} },
  // minGap sweep
  { name: "edge · 3:1 · minGap=0.35",                        params: basePs({ minGapAtrMul: 0.35 }),                                  filters: {} },
  { name: "edge · 3:1 · minGap=0.4",                         params: basePs({ minGapAtrMul: 0.4 }),                                   filters: {} },
  { name: "edge · 3:1 · minGap=0.6",                         params: basePs({ minGapAtrMul: 0.6 }),                                   filters: {} },
  { name: "edge · 3:1 · minGap=0.7",                         params: basePs({ minGapAtrMul: 0.7 }),                                   filters: {} },
  { name: "edge · 3:1 · minGap=1.0",                         params: basePs({ minGapAtrMul: 1.0 }),                                   filters: {} },
  // R:R sweep with minGap=0.5
  { name: "edge · 2:1 · minGap=0.5",                         params: basePs({ targetRMult: 2.0 }),                                    filters: {} },
  { name: "edge · 2.5:1 · minGap=0.5",                       params: basePs({ targetRMult: 2.5 }),                                    filters: {} },
  { name: "edge · 4:1 · minGap=0.5",                         params: basePs({ targetRMult: 4.0 }),                                    filters: {} },
  { name: "edge · 5:1 · minGap=0.5",                         params: basePs({ targetRMult: 5.0 }),                                    filters: {} },
  { name: "edge · 6:1 · minGap=0.5",                         params: basePs({ targetRMult: 6.0 }),                                    filters: {} },
  // Entry-depth combos
  { name: "ce · 3:1 · minGap=0.5",                           params: basePs({ entryDepth: 1 }),                                       filters: {} },
  { name: "ce · 4:1 · minGap=0.5",                           params: basePs({ entryDepth: 1, targetRMult: 4.0 }),                     filters: {} },
  { name: "ce · 5:1 · minGap=0.5",                           params: basePs({ entryDepth: 1, targetRMult: 5.0 }),                     filters: {} },
  { name: "far · 3:1 · minGap=0.5",                          params: basePs({ entryDepth: 2 }),                                       filters: {} },
  // Rejection filter
  { name: "edge · 3:1 · minGap=0.5 + rejection",             params: basePs({ requireRejection: 1 }),                                 filters: {} },
  { name: "ce · 3:1 · minGap=0.5 + rejection",               params: basePs({ entryDepth: 1, requireRejection: 1 }),                  filters: {} },
  // ADX gates
  { name: "edge · 3:1 · minGap=0.5 + maxAdx=22",             params: basePs(),                                                        filters: { maxAdx: 22 } },
  { name: "edge · 3:1 · minGap=0.5 + minAdx=22",             params: basePs(),                                                        filters: { minAdx: 22 } },
  { name: "edge · 3:1 · minGap=0.5 + with-trend@20",         params: basePs(),                                                        filters: { withTrendOnlyAboveAdx: 20 } },
  { name: "edge · 3:1 · minGap=0.5 + with-trend@22",         params: basePs(),                                                        filters: { withTrendOnlyAboveAdx: 22 } },
  // Best-guess combos
  { name: "ce · 4:1 · minGap=0.5 + with-trend@22",           params: basePs({ entryDepth: 1, targetRMult: 4.0 }),                     filters: { withTrendOnlyAboveAdx: 22 } },
  { name: "edge · 4:1 · minGap=0.6 + with-trend@22",         params: basePs({ targetRMult: 4.0, minGapAtrMul: 0.6 }),                 filters: { withTrendOnlyAboveAdx: 22 } },
  { name: "edge · 5:1 · minGap=0.5 + with-trend@22",         params: basePs({ targetRMult: 5.0 }),                                    filters: { withTrendOnlyAboveAdx: 22 } },
];

async function main() {
  const c = new C(); await c.ready;
  console.log(`[silver-fvg-refine] Silver / 1h × ${COUNT} bars · iteration 2 around minGap=0.5\n`);
  ...
  rows.filter((r) => r.trades >= 30).sort((a, b) => b.expR - a.expR).slice(0, 5).forEach((r) =>
    console.log(`  ${r.name.padEnd(54)}  ${String(r.trades).padStart(3)} trades · ${(100*r.wins/r.trades).toFixed(0)}% WR · expR=${(r.expR >= 0 ? "+" : "") + r.expR.toFixed(2)} · pnl=${(r.pnlUsd >= 0 ? "+" : "") + "$" + r.pnlUsd.toFixed(2)}`),
  );
