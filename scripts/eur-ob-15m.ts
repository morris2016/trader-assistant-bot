// EUR/USD OB deep-dive. First major-FX strategy in the registry.
// Tested with TRAIN/TEST split + warmup from the start to avoid the LTC overfit pattern:
//   TRAIN: ~3 months historical (Jan-Mar 2026)
//   TEST:  Apr 1 - Apr 27 OOS verification
// Both windows use bars before their start as warmup.
...
async function main() {
  ...
  console.log(`[eur-ob-15m] Symbol: EUR/USD (frxEURUSD) · cost ${COST_BPS} bps · 15m TF`);
  ...
  console.log(`TRAIN warmup: ${trainWarmup} bars (~${Math.round(trainWarmup/96)}d) before Jan 1`);
  console.log(`TEST warmup:  ${testWarmup} bars (~${Math.round(testWarmup/96)}d) before Apr 1\n`);
