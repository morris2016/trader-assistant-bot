// CRASH 300N/600/900 SELL-only — sell on confirmed down-spike (ride the spike,
// don't fade). Mirror of synth-boom-buyonly.ts.
...
// SELL-only: ride confirmed DOWN-spike on CRASH (instead of fading it).
function detect(candles: Candle[]): Sig[] {
  ...
  for (let i = ATR_PERIOD + 2; i < candles.length; i++) {
    ...
    if (!(spike.close < spike.open)) continue;       // bearish spike bar
    if (!(confirm.close < spike.close)) continue;    // confirmation bar continues down
...
async function main() {
  console.log(`CRASH SELL-only spike-ride (NOT fade) @ 1m`);
