// ETH FVG deep-dive. Reference points:
//   Silver FVG: minGap=0.7×ATR, edge, 3:1, no filter → +0.73R / +$1349 (winner)
//   BTC FVG: null (flat near zero across minGap sweep)
//   ETH OB: with-trend@20 + maxAdx=40 + skipSat + edge + 6:1 → +$729 (similar retest setup)
...
const variants: Variant[] = [
  // Phase A: minGap sweep (Silver-style monotonic check)
  ...
  // Phase B: R:R sweep at default minGap
  ...
  { name: "edge · 6:1 · minGap=0.15",                    params: basePs({ targetRMult: 6.0 }),                     filters: {} },
  // Phase C: ETH-OB-style filters applied to FVG
  ...
  { name: "edge · 6:1 + with-trend@20 + maxAdx=40",      params: basePs({ targetRMult: 6.0 }),                     filters: { withTrendOnlyAboveAdx: 20, maxAdx: 40 } },
  { name: "edge · 6:1 + with-trend@20 + skipSat",        params: basePs({ targetRMult: 6.0 }),                     filters: { withTrendOnlyAboveAdx: 20, skipDaysOfWeekUtc: [6] } },
  { name: "edge · 6:1 + ETH-OB filters (maxAdx40+skipSat)", params: basePs({ targetRMult: 6.0 }),                  filters: { withTrendOnlyAboveAdx: 20, maxAdx: 40, skipDaysOfWeekUtc: [6] } },
  // Phase D: entry depth
  { name: "ce · 3:1 · minGap=0.15",                      params: basePs({ entryDepth: 1 }),                        filters: {} },
  { name: "ce · 6:1 · minGap=0.15",                      params: basePs({ entryDepth: 1, targetRMult: 6.0 }),      filters: {} },
  { name: "far · 3:1 · minGap=0.15",                     params: basePs({ entryDepth: 2 }),                        filters: {} },
  // Phase E: rejection filter
  { name: "edge · 3:1 + rejection",                      params: basePs({ requireRejection: 1 }),                  filters: {} },
  // Phase F: combined high-quality champions
  { name: "edge · 6:1 · minGap=0.5",                     params: basePs({ targetRMult: 6.0, minGapAtrMul: 0.5 }),  filters: {} },
  { name: "edge · 6:1 · minGap=0.7",                     params: basePs({ targetRMult: 6.0, minGapAtrMul: 0.7 }),  filters: {} },
  { name: "edge · 5:1 · minGap=0.5 + with-trend@20",     params: basePs({ targetRMult: 5.0, minGapAtrMul: 0.5 }),  filters: { withTrendOnlyAboveAdx: 20 } },
  { name: "edge · 5:1 · minGap=0.7 + with-trend@20",     params: basePs({ targetRMult: 5.0, minGapAtrMul: 0.7 }),  filters: { withTrendOnlyAboveAdx: 20 } },
];
...
async function main() {
  ...
  console.log(`[eth-fvg-deepdive] ETH / 1h × 8000 bars · cost ${COST_BPS} bps\n`);
