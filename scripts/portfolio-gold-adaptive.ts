// Gold-only 30-day portfolio sim: 3 Gold strategies (OB + Sweep + FVG).
// All BUY-only. Walks balance trade-by-trade.
...
const MIN_STAKE = 50;
const MAX_STAKE_PCT = Number(process.env.MAX_PCT ?? 0.15);   // default 15% (half-Kelly), env-overridable
const MAX_EXPR_NORM = 0.73;        // highest validated expR in registry (Silver FVG)
const MULT = 30;
...
// Confidence-based adaptive sizing.
//   conf = validated expR / MAX_EXPR_NORM (clamped 0..1)
//   stake = max(MIN_STAKE, balance * MAX_STAKE_PCT * conf)
...
async function main() {
  ...
  console.log(`Fetching Gold 1h data...`);
  const c60Gold = await fetchPaged(c, "frxXAUUSD", 3600, 5000);
  c.close();
  console.log(`Gold 1h: ${c60Gold.length} bars (~${(c60Gold.length/24).toFixed(0)}d)\n`);

  console.log(`Running 3 Gold strategies (OB + Sweep + FVG)...`);
  const r1 = await runStrategy(goldOb,    c60Gold);
  const r2 = await runStrategy(goldSweep, c60Gold);
  const r3 = await runStrategy(goldFvg,   c60Gold);
  console.log(`Done.\n`);

  // expR confidence per strategy (from validation block).
  // gold_ob/sweep/fvg all hit +0.69R in validation.
  ...
  const all: Tagged[] = [
    ...r1.trades.map((t) => ({ t, tag: "G-OB ", cs: c60Gold, asset: "Gold" })),
    ...r2.trades.map((t) => ({ t, tag: "G-SW ", cs: c60Gold, asset: "Gold" })),
    ...r3.trades.map((t) => ({ t, tag: "G-FVG", cs: c60Gold, asset: "Gold" })),
  ];

  // Find the 30-day window — last DAYS days from the most recent epoch we have.
  const lastEpoch = c60Gold[c60Gold.length - 1].epoch;
  ...
  console.log(`══════════════════════════════════════════════════════════════════════════════════`);
  console.log(`Starting balance: $${START_BALANCE} · ADAPTIVE stake (min $${MIN_STAKE}, max ${(MAX_STAKE_PCT*100).toFixed(0)}% of balance, scaled by validated expR / ${MAX_EXPR_NORM}R) · ${MULT}× MULT`);
  console.log(`Confidence map: G-OB=${(expRByTag["G-OB "]/MAX_EXPR_NORM*100).toFixed(0)}% · G-SW=${(expRByTag["G-SW "]/MAX_EXPR_NORM*100).toFixed(0)}% · G-FVG=${(expRByTag["G-FVG"]/MAX_EXPR_NORM*100).toFixed(0)}% of max position`);
  ...
  console.log(`Strategies (3): gold_ob, gold_sweep, gold_fvg — Gold-only book, all BUY-only`);
  ...
  console.log(`${"#".padStart(3)}  ${"date".padEnd(18)} ${"strat".padEnd(5)} ${"side".padEnd(4)} ${"stake".padStart(7)} ${"R".padStart(7)} ${"$".padStart(8)}  ${"balance".padStart(9)} ${"peak".padStart(8)} ${"DD".padStart(5)}`);
  for (let i = 0; i < inWindow.length; i++) {
    ...
    const sign = usd >= 0 ? "+" : "";
    console.log(
      `${String(i + 1).padStart(3)}  ${opened.padEnd(18)} ${x.tag} ${x.t.side.padEnd(4)} $${stake.toFixed(0).padStart(5)} ${(r >= 0 ? "+" : "") + r.toFixed(2) + "R"}  ${sign}$${usd.toFixed(2).padStart(7)}  $${balance.toFixed(2).padStart(8)} $${peak.toFixed(2).padStart(7)} ${ddPct.toFixed(0).padStart(3)}%${flag}`,
    );
  }
