// BOOM 300N TP-fraction curve — WR vs EPD trade-off at fixed buf=0.05.
...
async function main() {
  console.log(`BOOM 300N TP-fraction curve at buf=0.05, spike=3.0 (Jan 1 → today)\n`);
  ...
  const TPS = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.2, 1.5, 2.0];
  console.log(`  TP×spike    TRAIN: trades  WR    epd      net           TEST: trades  WR    epd      net`);
  for (const tp of TPS) {
    const tr = sim(candles, tp, JAN_1_2025, splitEpoch);
    const te = sim(candles, tp, splitEpoch, TODAY);
    const flag = tp === 0.5 ? " ← current" : "";
    console.log(`  ${tp.toFixed(1)}×       ${String(tr.trades).padStart(5)}t  ${(tr.wr*100).toFixed(1).padStart(4)}%  +$${tr.epd.toFixed(3)}  +$${tr.net.toFixed(0).padStart(4)}        ${String(te.trades).padStart(5)}t  ${(te.wr*100).toFixed(1).padStart(4)}%  +$${te.epd.toFixed(3)}  +$${te.net.toFixed(0).padStart(4)}${flag}`);
  }
}
