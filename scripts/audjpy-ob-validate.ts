// AUD/JPY OB 3-window cross-validation.
// Primary config under test: orderBlock with minAdx=25, lookback=24, targetRMult=4, defaults elsewhere.
// Robustness checks: 4 plateau-neighbor configs to confirm parameter stability.
...
const GR = 1800; // 30m — 1h failed CV due to low trade count per window
const FETCH = 7000;
...
// 30m TF — round 3. BUY·3:1 was the cleanest 30m result (4W/0L/0F, +$53).
const configs: Config[] = [
  { name: "PRIMARY 30m: BUY·3:1",                   params: obParams(),                                     filters: { buyOnly: true } },
  { name: "robust 30m: BUY·4:1",                    params: obParams({ targetRMult: 4 }),                   filters: { buyOnly: true } },
  { name: "robust 30m: lookback=48·3:1",            params: obParams({ lookback: 48 }),                     filters: {} },
  { name: "robust 30m: BUY+lookback=48·3:1",        params: obParams({ lookback: 48 }),                     filters: { buyOnly: true } },
  { name: "robust 30m: BUY+minAdx=22·3:1",          params: obParams(),                                     filters: { buyOnly: true, minAdx: 22 } },
];
...
async function runWindow(cfg: Config, candles: Candle[], windowStart: number, windowEnd: number) {
  ...
  const detectors: DetectorConfig[] = defaultDetectorConfigs().map((d) => ({
    ...
    params: d.id === "orderBlock" ? cfg.params : d.params,
  }));
...
async function main() {
  ...
  console.log(`AUD/JPY OB 3-window cross-validation\n`);
  ...
  if (primary.strong) console.log(`✓✓ PRIMARY config PASSES STRONG threshold — register as audjpy_ob.`);
  else if (primary.passes) console.log(`✓ PRIMARY config passes 3-window CV but combined $${primary.total.toFixed(0)} < $${STRONG_THRESHOLD} STRONG threshold.`);
  else console.log(`✗ PRIMARY config FAILS 3-window CV.`);
