// AUD/JPY OB on 15m — testing if lower TF gives more trades for stable validation.
// 1 bar = 15 min, so lookback=24 = 6h, lookback=96 = 24h. Variant axes scaled for 15m.
...
// 15m exploration: 1 bar = 15 min. lookback=24 = 6h. lookback=96 = 24h. lookback=192 = 48h.
const variants: Variant[] = [
  // baseline anchor (15m raw)
  ...
  // lookback scaled for 15m
  { name: "lookback=48·3:1 (12h)",                 params: obParams({ lookback: 48 }),                                              filters: {} },
  { name: "lookback=96·3:1 (24h)",                 params: obParams({ lookback: 96 }),                                              filters: {} },
  { name: "lookback=192·3:1 (48h)",                params: obParams({ lookback: 192 }),                                             filters: {} },
  // obSearchMaxBack
  ...
  { name: "BUY+lookback=96·4:1",                   params: obParams({ lookback: 96, targetRMult: 4 }),                              filters: { buyOnly: true } },
  { name: "minAdx=25+lookback=96·4:1",             params: obParams({ lookback: 96, targetRMult: 4 }),                              filters: { minAdx: 25 } },
  { name: "BUY+minAdx=25+lookback=96·4:1",         params: obParams({ lookback: 96, targetRMult: 4 }),                              filters: { buyOnly: true, minAdx: 25 } },
];
...
async function main() {
  ...
  console.log(`AUD/JPY OB month-by-month walk-forward · ${variants.length} variants · 15m\n`);
