...
const variants: Variant[] = [
  // ITER1 WIN: edge · 4.5:1 + BUY = 108t / 31% WR / +0.19R / +$463.89
  { name: "WIN-iter1: edge · 4.5:1 + BUY",               params: basePs({ targetRMult: 4.5 }),                     filters: { buyOnly: true } },
  // R:R fine sweep around 4.5
  { name: "edge · 4.0 + BUY",                            params: basePs({ targetRMult: 4.0 }),                     filters: { buyOnly: true } },
  { name: "edge · 4.25 + BUY",                           params: basePs({ targetRMult: 4.25 }),                    filters: { buyOnly: true } },
  { name: "edge · 4.75 + BUY",                           params: basePs({ targetRMult: 4.75 }),                    filters: { buyOnly: true } },
  { name: "edge · 5.0 + BUY",                            params: basePs({ targetRMult: 5.0 }),                     filters: { buyOnly: true } },
  // Stack BUY + 4.5 with filters
  { name: "4.5 + BUY + skipSat",                         params: basePs({ targetRMult: 4.5 }),                     filters: { buyOnly: true, skipDaysOfWeekUtc: [6] } },
  { name: "4.5 + BUY + skipSatSun",                      params: basePs({ targetRMult: 4.5 }),                     filters: { buyOnly: true, skipDaysOfWeekUtc: [0, 6] } },
  { name: "4.5 + BUY + with-trend@20",                   params: basePs({ targetRMult: 4.5 }),                     filters: { buyOnly: true, withTrendOnlyAboveAdx: 20 } },
  { name: "4.5 + BUY + minAdx=22",                       params: basePs({ targetRMult: 4.5 }),                     filters: { buyOnly: true, minAdx: 22 } },
  { name: "4.5 + BUY + minAdx=18",                       params: basePs({ targetRMult: 4.5 }),                     filters: { buyOnly: true, minAdx: 18 } },
  { name: "4.5 + BUY + maxAdx=40",                       params: basePs({ targetRMult: 4.5 }),                     filters: { buyOnly: true, maxAdx: 40 } },
  { name: "4.5 + BUY + maxAdx=50",                       params: basePs({ targetRMult: 4.5 }),                     filters: { buyOnly: true, maxAdx: 50 } },
  // Detector params
  { name: "4.5 + BUY + maxActive=20",                    params: basePs({ targetRMult: 4.5, maxActive: 20 }),      filters: { buyOnly: true } },
  { name: "4.5 + BUY + maxActive=6",                     params: basePs({ targetRMult: 4.5, maxActive: 6 }),       filters: { buyOnly: true } },
  { name: "4.5 + BUY + buf=0.05",                        params: basePs({ targetRMult: 4.5, stopBufferAtrMul: 0.05 }), filters: { buyOnly: true } },
  { name: "4.5 + BUY + buf=0.2",                         params: basePs({ targetRMult: 4.5, stopBufferAtrMul: 0.2 }), filters: { buyOnly: true } },
  { name: "4.5 + BUY + atrP=10",                         params: basePs({ targetRMult: 4.5, atrPeriod: 10 }),      filters: { buyOnly: true } },
  { name: "4.5 + BUY + atrP=20",                         params: basePs({ targetRMult: 4.5, atrPeriod: 20 }),      filters: { buyOnly: true } },
  // requireRejection (was +$161 with smaller sample at 4:1)
  { name: "4.5 + BUY + requireRejection",                params: basePs({ targetRMult: 4.5, requireRejection: 1 }), filters: { buyOnly: true } },
  // Wider minGap variations stay
  { name: "4.5 + BUY + minGap=0.2",                      params: basePs({ targetRMult: 4.5, minGapAtrMul: 0.2 }),  filters: { buyOnly: true } },
  { name: "4.5 + BUY + minGap=0.25",                     params: basePs({ targetRMult: 4.5, minGapAtrMul: 0.25 }), filters: { buyOnly: true } },
  { name: "4.5 + BUY + minGap=0.10",                     params: basePs({ targetRMult: 4.5, minGapAtrMul: 0.10 }), filters: { buyOnly: true } },
];
