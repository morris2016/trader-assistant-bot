// R_100 FVG 3-window cross-validation.
// Primary config: fvg with minGapAtrMul=0.20, ADX band [18,38], targetRMult=3, defaults elsewhere.
// Robustness checks: plateau-neighbor configs.
...
const configs: Config[] = [
  { name: "PRIMARY: minGap=0.20+adx[18,38]·3:1",    params: fvgParams({ minGapAtrMul: 0.20 }),              filters: { minAdx: 18, maxAdx: 38 } },
  { name: "robust: adx[18,38]·3:1",                 params: fvgParams(),                                    filters: { minAdx: 18, maxAdx: 38 } },
  { name: "robust: adx[18,38]+atr=21",              params: fvgParams({ atrPeriod: 21 }),                   filters: { minAdx: 18, maxAdx: 38 } },
  { name: "robust: adx[18,36]·3:1",                 params: fvgParams(),                                    filters: { minAdx: 18, maxAdx: 36 } },
  { name: "robust: adx[18,38]+minGap=0.10",         params: fvgParams({ minGapAtrMul: 0.10 }),              filters: { minAdx: 18, maxAdx: 38 } },
];
...
async function runWindow(cfg: Config, candles: Candle[], windowStart: number, windowEnd: number) {
  ...
  const detectors: DetectorConfig[] = defaultDetectorConfigs().map((d) => ({
    ...d, enabled: d.id === "fvg",
    params: d.id === "fvg" ? cfg.params : d.params,
  }));
...
async function main() {
  ...
  console.log(`R_100 FVG 3-window cross-validation\n`);
  ...
  if (primary.strong) console.log(`✓✓ PRIMARY config PASSES STRONG threshold — register as r100_fvg.`);
