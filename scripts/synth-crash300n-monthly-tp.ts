// CRASH 300N monthly TP comparison — same detector, BUY-side fade, verify TP=0.7 is also robust.
...
function sim(candles: Candle[], tpFrac: number, from: number, to: number) {
  ...
  for (let i = ATR_PERIOD + 2; i < candles.length; i++) {
    ...
    if (!(spike.close < spike.open)) continue;     // bearish spike
    if (!(confirm.close > spike.close)) continue;  // confirm reversal up
    if (i + 1 >= candles.length) continue;
    const finBar = candles[i + 1]; const finalE = confirm.close + confirm.close * ENTRY_SPREAD_FRAC;
    const stop = spike.low - BUFFER_ATR * a; const target = confirm.close + tpFrac * range;
    ...
    if (!exit) continue;
    const move = (exitPrice - finalE) / finalE; let pnl = STAKE * MULT * move - STAKE * COMMISSION_FRAC;
...
async function main() {
  console.log(`CRASH 300N — per-month TP comparison (buf=0.05, spike=3.0)\n`);
  ...
  let y = firstD.getUTCFullYear(), m = firstD.getUTCMonth();
  ...
  while (true) {
    ...
    for (const tp of TPS) {
      ...
      totals[`${tp}`].trades += r.trades; totals[`${tp}`].wins += r.wins; totals[`${tp}`].net += r.net;
      cells.push(`${String(r.trades).padStart(4)}t ${(r.wr*100).toFixed(0).padStart(2)}% ${r.net >= 0 ? "+" : ""}$${r.net.toFixed(0).padStart(4)}`);
    }
    console.log(`  ${y}-${String(m + 1).padStart(2, "0")}      ${cells.join("    ")}`);
    if (m === 11) { y++; m = 0; } else m++;
  }
  console.log(`  ──────────`);
  const totalCells = TPS.map((tp) => { const t = totals[`${tp}`]; const wr = t.trades > 0 ? t.wins / t.trades : 0; return `${String(t.trades).padStart(4)}t ${(wr*100).toFixed(0).padStart(2)}% ${t.net >= 0 ? "+" : ""}$${t.net.toFixed(0).padStart(4)}`; });
