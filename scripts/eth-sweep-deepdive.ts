// ETH Sweep deep-dive — test both ICT-style (Silver pref) and confirmation-style (BTC pref) entries.
// 1h focus, full filter sweep.
...
type Variant = {
  ...
  filters: Partial<{ maxAdx: number; minAdx: number; withTrendOnlyAboveAdx: number; skipDaysOfWeekUtc: number[] }>;
};

const variants: Variant[] = [
  // Phase A: entry style head-to-head
  { name: "ICT 3:1 (Silver-style)",                       params: basePs(),                                              filters: {} },
  { name: "confirm 3:1 (BTC-style)",                      params: basePs({ entryOnSweep: 0 }),                           filters: {} },
  ...
  // Phase B: R:R sweep on best entry style
  ...
  { name: "confirm 6:1 + withTrend@20",                   params: basePs({ entryOnSweep: 0, targetRMult: 6.0 }),         filters: { withTrendOnlyAboveAdx: 20 } },
  // Phase C: ADX gate variations
  { name: "ICT 4:1 + withTrend@22",                       params: basePs({ targetRMult: 4.0 }),                          filters: { withTrendOnlyAboveAdx: 22 } },
  { name: "ICT 4:1 + withTrend@18",                       params: basePs({ targetRMult: 4.0 }),                          filters: { withTrendOnlyAboveAdx: 18 } },
  { name: "ICT 4:1 + maxAdx=22 (ranging)",                params: basePs({ targetRMult: 4.0 }),                          filters: { maxAdx: 22 } },
  { name: "ICT 4:1 + minAdx=22 (trending only)",          params: basePs({ targetRMult: 4.0 }),                          filters: { minAdx: 22 } },
  // Phase D: ETH-OB-borrowed filters (maxAdx=40 + skipSat)
  { name: "ICT 4:1 + withTrend@20 + maxAdx=40",           params: basePs({ targetRMult: 4.0 }),                          filters: { withTrendOnlyAboveAdx: 20, maxAdx: 40 } },
  { name: "ICT 4:1 + withTrend@20 + skipSat",             params: basePs({ targetRMult: 4.0 }),                          filters: { withTrendOnlyAboveAdx: 20, skipDaysOfWeekUtc: [6] } },
  { name: "ICT 4:1 + withTrend@20 + maxAdx=40 + skipSat", params: basePs({ targetRMult: 4.0 }),                          filters: { withTrendOnlyAboveAdx: 20, maxAdx: 40, skipDaysOfWeekUtc: [6] } },
];

async function main() {
  const c = new C(); await c.ready;
  console.log(`[eth-sweep-deepdive] Symbol: Ethereum / USD · cost ${COST_BPS} bps\n`);
