// RDBEAR breakout 5m — BOTH SIDES (sideFilter=0) with static martingale.
// "Trade with the local regime": when RDBEAR pierces prior N-bar HIGH → BUY,
// when it pierces prior LOW → SELL. The breakout direction IS the regime
// signal. Combine with martingale to compound edge during sustained moves.
//
// Iterates over (kAtr, momR) variants until one shows a real edge across
// the 4dago bad window.
...
// Sweep across kAtr/momR combinations to find an edge with both sides allowed.
...
const VARIANTS: Variant[] = [
  ...
  { name: "lb15 kAtr2.0 momR0.80", lookback: 15, kAtr: 2.0, momRatio: 0.80 },
  { name: "lb15 kAtr2.5 momR0.80", lookback: 15, kAtr: 2.5, momRatio: 0.80 },
  { name: "lb20 kAtr2.0 momR0.70", lookback: 20, kAtr: 2.0, momRatio: 0.70 },
  { name: "lb20 kAtr2.5 momR0.70", lookback: 20, kAtr: 2.5, momRatio: 0.70 },
  { name: "lb15 kAtr3.0 momR0.70", lookback: 15, kAtr: 3.0, momRatio: 0.70 },
  { name: "lb10 kAtr2.0 momR0.65", lookback: 10, kAtr: 2.0, momRatio: 0.65 },
];
...
// Both-sides breakout: pierce of prior N-bar HIGH (with momentum) → BUY,
// pierce of prior N-bar LOW (with momentum) → SELL.
function detect(candles: Candle[], v: Variant): Sig[] {
  const out: Sig[] = [];
  const atrPeriod = 14;
  for (let i = Math.max(v.lookback, atrPeriod) + 1; i < candles.length; i++) {
...
type Trade = { sigT: string; closeT: string; side: "BUY" | "SELL"; level: number; stake: number; exit: "tp" | "sl"; net: number; bal: number };
...
function honestSim(candles: Candle[], v: Variant) {
  ...
  for (const sig of sigs) {
    ...
    // For BUY: adverse spread = higher fill (price up). For SELL: lower fill.
    ...
    trades.push({
      ...
      side: sig.side, level: tradeLevel, stake, exit, net, bal: balance,
    });
  }
  return { trades, bust, finalBal: balance, peak, trough, buyCount, sellCount };
}

async function main() {
  const dateStr = new Date(TARGET_DAY_START * 1000).toISOString().slice(0, 10);
  console.log(`HONEST sim — RDBEAR breakout 5m  BOTH SIDES + MART ${MART}× × ${MAX_LEVELS}L`);
  console.log(`${dateStr} ${START_HOUR}:00→${END_HOUR}:00 UTC  (${(END_HOUR-START_HOUR)}h)`);
  console.log(`ACCT=$${ACCT}  STAKE=$${BASE_STAKE}  MULT=${MULT}×  CAP=$${PER_TRADE_CAP}\n`);
  ...
  console.log(`══════════════════════════════════════════════════════════════════════════════════════`);
  console.log(`  variant                   trades  B/S      W/L     WR     final   Δ%      peak    trough  result`);
  console.log(`══════════════════════════════════════════════════════════════════════════════════════`);
  for (const v of VARIANTS) {
    ...
    console.log(`  ${v.name.padEnd(24)}  ${String(r.trades.length).padStart(3)}t  ${String(r.buyCount).padStart(3)}/${String(r.sellCount).padStart(3)}  ${wins}W/${losses}L   ${(wr*100).toFixed(0).padStart(2)}%  $${r.finalBal.toFixed(2).padStart(7)}  ${dPct.padStart(4)}%  $${r.peak.toFixed(0).padStart(4)}  $${r.trough.toFixed(0).padStart(4)}  ${status}`);
  }
}
