// Palladium Sweep month-by-month walk-forward — quick survey.
...
// Iter 5: micro-tune around best+lb=60 (+$974, 5/0/0). Single boosters > compounds.
const baseBest = { targetRMult: 4, stopBufferAtrMul: 0.25, equalToleranceAtrMul: 0.18 };
const variants: Variant[] = [
  // carry-overs
  { name: "iter3-best: sb0.25·4:1+eq0.18", params: swParams(baseBest),                            filters: {} },
  { name: "iter3-#2: sb0.30·4:1+eq0.20+skipFri", params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.30, equalToleranceAtrMul: 0.20 }), filters: { skipDaysOfWeekUtc: [5] } },
  { name: "iter3-#3: sb0.30·4:1+eq0.20+lb=80", params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.30, equalToleranceAtrMul: 0.20, lookbackBars: 80 }), filters: {} },
  // eqTol fine fill around 0.18
  { name: "sb0.25·4:1+eq0.16",             params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.25, equalToleranceAtrMul: 0.16 }), filters: {} },
  { name: "sb0.25·4:1+eq0.17",             params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.25, equalToleranceAtrMul: 0.17 }), filters: {} },
  { name: "sb0.25·4:1+eq0.19",             params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.25, equalToleranceAtrMul: 0.19 }), filters: {} },
  // best + skipFri
  { name: "best + skipFri",                params: swParams(baseBest),                            filters: { skipDaysOfWeekUtc: [5] } },
  // best + lb variations
  { name: "best + lb=60",                  params: swParams({ ...baseBest, lookbackBars: 60 }),   filters: {} },
  { name: "best + lb=80",                  params: swParams({ ...baseBest, lookbackBars: 80 }),   filters: {} },
  { name: "best + lb=100",                 params: swParams({ ...baseBest, lookbackBars: 100 }),  filters: {} },
  // best + atr variations
  { name: "best + atr=18",                 params: swParams({ ...baseBest, atrPeriod: 18 }),      filters: {} },
  { name: "best + atr=21",                 params: swParams({ ...baseBest, atrPeriod: 21 }),      filters: {} },
  { name: "best + atr=28",                 params: swParams({ ...baseBest, atrPeriod: 28 }),      filters: {} },
  // best + R:R variations
  { name: "sb0.25·3.5:1+eq0.18",           params: swParams({ ...baseBest, targetRMult: 3.5 }),   filters: {} },
  { name: "sb0.25·4.5:1+eq0.18",           params: swParams({ ...baseBest, targetRMult: 4.5 }),   filters: {} },
  // best + bigger stopBuf with eq0.18
  { name: "sb0.30·4:1+eq0.18",             params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.30, equalToleranceAtrMul: 0.18 }), filters: {} },
  { name: "sb0.35·4:1+eq0.18",             params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.35, equalToleranceAtrMul: 0.18 }), filters: {} },
  // TRIPLE compound: eq0.18 + skipFri + lb=80
  { name: "best + skipFri + lb=80",        params: swParams({ ...baseBest, lookbackBars: 80 }),   filters: { skipDaysOfWeekUtc: [5] } },
  { name: "best + skipFri + atr=21",       params: swParams({ ...baseBest, atrPeriod: 21 }),      filters: { skipDaysOfWeekUtc: [5] } },
  { name: "best + lb=80 + atr=21",         params: swParams({ ...baseBest, lookbackBars: 80, atrPeriod: 21 }), filters: {} },
  // QUAD compound
  { name: "best + skipFri + lb=80 + atr=21", params: swParams({ ...baseBest, lookbackBars: 80, atrPeriod: 21 }), filters: { skipDaysOfWeekUtc: [5] } },
  // sb0.30 quad variants
  { name: "sb0.30+eq0.20+skipFri+lb=80",   params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.30, equalToleranceAtrMul: 0.20, lookbackBars: 80 }), filters: { skipDaysOfWeekUtc: [5] } },
  { name: "sb0.30+eq0.20+skipFri+atr=21",  params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.30, equalToleranceAtrMul: 0.20, atrPeriod: 21 }), filters: { skipDaysOfWeekUtc: [5] } },
  { name: "sb0.30+eq0.20+lb=80+atr=21",    params: swParams({ targetRMult: 4, stopBufferAtrMul: 0.30, equalToleranceAtrMul: 0.20, lookbackBars: 80, atrPeriod: 21 }), filters: {} },
];
...
async function main() {
  ...
  console.log(`Palladium/USD Sweep month-by-month walk-forward · ${variants.length} variants · 1h\n`);
