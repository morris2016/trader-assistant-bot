// R_100 (Volatility 100 Index) OB — pure synthetic, no news, 24/7, unlimited history.
// Properties: smooth Brownian-like with ~100% annualized vol, no real liquidity providers,
// no regime shifts from macro. SMC structures form mathematically without institutional bias.
// Hypotheses: defaults may work fine (no regime issues); side bias is irrelevant (random walk);
// shorter retest expected (no news lag); displacement = pure ATR-relative.
...
// R_100 OB single-axis exploration from raw obParams() defaults.
// Reasoning: synthetic random walk → no side bias matters; clean ATR scaling; no news.
const variants: Variant[] = [
  // baseline anchor (raw defaults)
  { name: "raw default·3:1",                       params: obParams(),                                                              filters: {} },
  // R:R sweep
  { name: "raw·2:1",                               params: obParams({ targetRMult: 2 }),                                            filters: {} },
  { name: "raw·2.5:1",                             params: obParams({ targetRMult: 2.5 }),                                          filters: {} },
  { name: "raw·4:1",                               params: obParams({ targetRMult: 4 }),                                            filters: {} },
  { name: "raw·5:1",                               params: obParams({ targetRMult: 5 }),                                            filters: {} },
  // displacement (ATR-scaled, should be the natural axis)
  ...
  // lookback (synthetic structure may differ)
  { name: "lookback=8·3:1",                        params: obParams({ lookback: 8 }),                                               filters: {} },
  { name: "lookback=20·3:1",                       params: obParams({ lookback: 20 }),                                              filters: {} },
  { name: "lookback=30·3:1",                       params: obParams({ lookback: 30 }),                                              filters: {} },
  // ADX bands (synthetic ADX behaves differently — random walk has cyclical ADX)
  ...
  // confirmation filters (no news = no sudden surprises, may need less confirmation)
  ...
  // synthetic-specific: stronger rejection body (clean candles)
  { name: "rejBody=0.5·3:1",                       params: obParams({ rejectionBodyAtrMul: 0.5 }),                                  filters: {} },
  // synthetic-specific: tight quality filter (random walks can be choppy)
  { name: "qualityLB=5·3:1",                       params: obParams({ qualityFilterLookback: 5 }),                                  filters: {} },
  // confluence (might overfilter on synthetic, worth testing)
  { name: "+FVG·3:1",                              params: obParams({ requireFVG: 1 }),                                             filters: {} },
  { name: "+Sweep·3:1",                            params: obParams({ requireLiquiditySweep: 1 }),                                  filters: {} },
  // higher R:R confirmation
  { name: "raw·6:1",                               params: obParams({ targetRMult: 6 }),                                            filters: {} },
];
...
async function main() {
  ...
  console.log(`R_100 OB month-by-month walk-forward · ${variants.length} variants · 1h\n`);
