// Fast-trade spike-fade screener for BOOM/CRASH 50N and 150N at 1m.
// These have faster spike cadence than 300N (every ~50/150 ticks vs ~300),
// so signals fire 3-6x more often. Same detector params as the validated
// Fast2 BOOM/CRASH 300N spike-fade strategy.
...
// Fast2-validated spike-fade params: 3.0 ATR spike, 0.2 buffer, 50% TP, confirmation=1
const SPIKE_NATR = 3.0;
const BUFFER_ATR = 0.2;
const TP_FRAC_OF_SPIKE = 0.5;
const ATR_PERIOD = 14;

// All BOOM/CRASH cadence variants — 300N (validated baseline), 600, 900.
const ASSETS = [
  { sym: "BOOM300N", gr: 60, side: "SELL" as const },
  { sym: "BOOM600",  gr: 60, side: "SELL" as const },
  { sym: "BOOM900",  gr: 60, side: "SELL" as const },
  { sym: "CRASH300N",gr: 60, side: "BUY" as const },
  { sym: "CRASH600", gr: 60, side: "BUY" as const },
  { sym: "CRASH900", gr: 60, side: "BUY" as const },
];
...
// Spike-fade detector: range ≥ N×ATR + confirmation bar, fade direction.
function detect(candles: Candle[], side: "BUY" | "SELL"): Sig[] {
  const out: Sig[] = [];
  for (let i = ATR_PERIOD + 2; i < candles.length; i++) {
    const a = atr(candles, i - 1, ATR_PERIOD); // ATR up to PRIOR bar
    if (a <= 0) continue;
    const spike = candles[i - 1]; // candidate spike bar
    const range = spike.high - spike.low;
    if (range < SPIKE_NATR * a) continue;
    const confirm = candles[i];
    if (side === "SELL") {
      // Fade UP-spike (BOOM): require confirmation bar closes lower than spike close
      if (!(spike.close > spike.open)) continue; // spike must be bullish bar
      ...
      // Fade DOWN-spike (CRASH): confirmation bar closes higher than spike close
...
async function main() {
  console.log(`Spike-fade screener — BOOM/CRASH 300N + 600 + 900 @ 1m (validated stack)`);
  ...
  for (const a of ASSETS) {
    for (const win of WINDOWS) {
      ...
      const r = honestSim(candles, a.side, ws_, we_);
