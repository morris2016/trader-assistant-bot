...
import { api, fmtTime, type ClosedPaperPosition, type EquityPoint, type PaperPosition, type PaperResp } from "../api";

export function PaperPanel({ doAction, pending }: {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [p, t, e] = await Promise.all([api.paper(), api.paperTrades(500), api.paperEquity()]);
        setPaper(p); setTrades(t.trades); setEquity(e.equity); setError(null);
  ...
  return (
    <>
      ...
      {paper.open.length > 0 && (
        <div className="section">
          <div className="section-header">
            <div className="section-title">Open Paper Positions ({paper.open.length})</div>
          </div>
          <div className="card table-card">
            <table>
              <thead>
                <tr>
                  <th>Opened</th><th>Symbol</th><th>Side</th><th>Detector</th><th>Stake</th><th>Entry</th><th>Stop</th><th>TP</th><th>Shift</th>
                </tr>
              </thead>
              <tbody>
      ...
      <div className="section">
        <div className="section-header">
          <div className="section-title">Closed Paper Trades ({filtered.length})</div>
        </div>
        <div className="card table-card">
          ...
          {filtered.length === 0 ? (
            <div className="empty"><span className="empty-emoji">📊</span>No paper trades yet — bot is dry-run; signals will trigger paper trades.</div>
          ) : (
            <table>
              <thead>
                <tr>
      ...
      <div className="section">
        <div className="section-header">
          <div className="section-title">Reset Paper Account</div>
          <div className="section-sub">Wipe paper balance + trade history + adaptive shift, restart at fresh balance</div>
        </div>
        <div className="card card-padded">
          <div className="row">
            <input
              ...
              style={{ width: 120 }}
            />
            <button className="btn btn-warn" disabled={pending !== null} onClick={() => doAction(`Reset paper account to $${resetTo}?`, () => api.resetPaper(Number(resetTo)))}>
              ⟳ Reset Paper
            </button>
          </div>
        </div>
...
function OpenRow({ p }: { p: PaperPosition }) {
  return (
    <tr>
      ...
      <td><span className="strat-chip">{p.appliedShiftReasons}</span></td>
    </tr>
  );
}
